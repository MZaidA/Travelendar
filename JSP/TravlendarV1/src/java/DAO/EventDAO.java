...
//    }
    ...
    public static int save(Event event) {
        ...
        try {
            Connection con = getConnection();
            PreparedStatement ps = con.prepareStatement("INSERT INTO event(USERNAME, EVENT_NAME, ARRIVAL_TIME, EVENT_END, DEPARTURE_TIME, ORIGIN, DESTINATION, TRAVEL_MODE, AVOID_TOLLS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            ps.setDate(5, new Date(event.getDepartureTime().getTime()));
